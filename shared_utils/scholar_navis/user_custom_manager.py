...
def get_api_key(user_custom_data: callable_dict,
                provider_api_type: Literal[
                    'API_KEY', 'XAI_API_KEY', 'GEMINI_API_KEY', 'ZHIPUAI_API_KEY', 'DASHSCOPE_API_KEY', 'MOONSHOT_API_KEY', 'DEEPSEEK_API_KEY']
                , allow_get_config: bool = True) -> str | list[str] | bool:
    """获取其他提供商的api-key（含自定义的API）

    Args:
        user_custom_data (dict): 用户自定义数据
        provider (str): key的提供商
        allow_get_config (bool, optional): 允许获取默认值吗（如有）.
    ...
    if not allow_get_config or a:
        return a
    else:
        return get_conf(provider_api_type)
...
def get_url_redirect_domain(redirect_type: str, user_custom_data: Union[str, dict]):
    ...
    if type(user_custom_data) is dict:
        dict_ = user_custom_data
    else:
        dict_ = _load_dict_from_str(user_custom_data)
