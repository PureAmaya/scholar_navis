...
def _is_openai_model(model_name):
    ...
    return model_name == 'o1' or model_name == 'o3' or model_name.startswith('gpt-') or model_name.startswith(
        'o1-') or model_name.startswith('o3-')
...
def what_keys(keys):
    ...
    return "Detected: OpenAI Key {}, Azure Key {}, API2D Key {}".format(avail_key_list['OpenAI Key'],
                                                                        avail_key_list['Azure Key'],
                                                                        avail_key_list['API2D Key'])
...
def select_api_key(keys, llm_model):
    ...
    if len(avail_key_list) == 0:
        raise RuntimeError(
            "The API key you provided does not meet the requirements and does not contain any valid API keys for {}. You may have selected the wrong model or request source (you can switch to other models or request sources in the top-left model selection menu).").format(
            llm_model)
