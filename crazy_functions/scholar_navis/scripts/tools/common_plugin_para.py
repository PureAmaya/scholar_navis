...
class common_plugin_para(GptAcademicPluginTemplate):
    ...
    def __link_command(self,command:str,command_natural_language:str,need_para: bool):
        ...
        if need_para:
            return f'{command}: {command_natural_language} ({NEED_PARA_INDICATOR})'
        else:
            return f'{command}: {command_natural_language}'
        
    def add_file_upload_field(self,title: str=_('文件上传路径或网址'),description : str= _('请上传文件后，再点击该插件')):
    ...
    def add_command_para_field(self,title=_('指令参数'),description = _('如需要，在此处输入辅助指令的参数')):
        """如果有的命令需要参数，再加上这个
    ...
    def add_lib_field(self,always_field:bool,title:str=_('总结库'),description:str = _('选择总结库进行操作')):
