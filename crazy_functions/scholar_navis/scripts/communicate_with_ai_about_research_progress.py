...
def 与AI交流研究进展(txt: str, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    history.append(
        _('如果有问道某些内容的出处、来源，或者是其他的你不知道的内容、没有给你提供的内容时，请说“如果要确定详细的内容或者是寻找来源，请使用 find 命令 和 精细分析文献 功能'))
    ...
    # 希望绘制思维导图等
        ...
        try:
            ...
            yield from update_ui_lastest_msg(
                _('参数 <b>{}</b> 不合适或超出范围，修正为 9（思维导图）').format(command_para), chatbot=chatbot,
                history=history)
            index = 9

        # 尝试调用方法，反正出问题会提醒
        try:
            yield from 生成多种Mermaid图表(summarization, llm_kwargs,
                                           {'index': index, 'gpt_prefer_lang': GPT_prefer_language}, chatbot, history,
                                           lang)
        except Exception as e:
    ...
    # 从总结的内容中找到自己感兴趣的文章
        ...
        if command_para == '':
            ...
            # 尝试调用方法，反正出问题会提醒
            try:
                yield from __find_article_from_summarization(this_library_fp, command_para, GPT_prefer_language,
                                                             llm_kwargs, system_prompt, chatbot, history)
            except Exception as e:
                yield from update_ui_lastest_msg(_('无法溯源。具体原因如下: \n{}').format(str(e)), chatbot=chatbot,
                                                 history=history)
            return
    ...
    # 想拟定几个课题
        ...
        try:
            ...
            yield from __come_up_with_topic(request, summarization, GPT_prefer_language, llm_kwargs, system_prompt,
                                            chatbot, history)
...
def __find_article_from_summarization(library_root_dir: str, txt: str, gpt_prefer_lang: str, llm_kwargs, system_prompt,
                                      chatbot, history):
    ...
    history_array = []
    sys_prompt_array = []
    _ = lambda text: init_language(text, chatbot.get_language())
    ...
    for index, content in enumerate(contents):
        ...
        sys_prompt_array.append(
            f"Make sure to let me know the reason for such a decision. Please respond in {gpt_prefer_lang}.")
    ...
    if len(filenames_meet_requirement) == 0:
        yield from update_ui_lastest_msg(
            _("搜索完成！暂时没有找到与搜索内容相匹配的文章。可以多尝试几次（<b>不排除AI出问题</b>），或与AI对总结内容进行交流")
            , chatbot, history)
        return
    ...
    line1 = _('搜索完成！')
    line2 = _(
        '有 <b>{}</b> 篇文章可能符合期望。在下面可以下载这些文章（以及选中的理由），此外现在<b>可以直接进行对话</b>、了解<b>这几篇</b>文章了').format(
        len(filenames_meet_requirement))
...
def __come_up_with_topic(request: str, summarization: str, GPT_prefer_language, llm_kwargs, system_prompt, chatbot,
                         history):
