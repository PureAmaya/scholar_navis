...
def start_app(app_block, authenticator,CONCURRENT_COUNT, AUTHENTICATION, PORT, SSL_KEYFILE, SSL_CERTFILE):
    ...
    def validate_path_safety(path_or_url, user):
        ...
        if path_or_url.startswith(PATH_LOGGING):    # 日志文件（按用户划分）
            ...
            return True # 允许访问主题文件夹
        else:
            raise FriendlyException(_("您没有访问该目录/文件的权限")) # return False
        
        if sensitive_path:
            allowed_users = [user,'shared']  # 仅允许访问自身文件夹和共享文件夹
    ...
    async def file(path_or_url: str,user = Depends(get_user)):
        ...
        if not os.path.exists(path_or_url):
            return PlainTextResponse(content=f'{_("无法找到该文件。可能由于过时被清除了，也可能从来没有存在过。")}\n\n404 Not Found.',status_code=404)
        try:
    ...
    if AUTHENTICATION:gradio_app = gr.mount_gradio_app(gradio_app,authenticator, path=auth_path)
